...
class CustomNetwork(torch.nn.Module):
    def __init__(self, network_input_arg, visual_prompt_external_arg,
        ...
        if self.method == 'lor_vp':
            ...
            if not (hasattr(self.network, 'original_network') and hasattr(self.network, 'visual_prompt') and hasattr(
                    self.network, 'rank')):
                pass
            else:
                lora_wrapper = self.network
                internal_vp_module = lora_wrapper.visual_prompt
                if isinstance(internal_vp_module, nn.Module):
                    for param in internal_vp_module.parameters():
                        param.requires_grad = True

                if lora_wrapper.rank > 0:
