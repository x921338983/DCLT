import os
from torch.utils.data import DataLoader, Subset, ConcatDataset, Dataset, RandomSampler, DistributedSampler, SequentialSampler, Sampler, random_split
from torchvision.datasets import CIFAR10, CIFAR100, SVHN, GTSRB, Food101, SUN397, EuroSAT, UCF101, DTD, OxfordIIITPet, \
    MNIST, ImageFolder, Flowers102, StanfordCars
...
def get_model(net, args):
    ...
    if net == "resnet18":
        if args.pretrain_path:
            ...
            network.load_state_dict(torch.load("../models/resnet18.pth"))
        else:
        ...
        if args.pretrain_path:
            ...
            network.load_state_dict(torch.load("../models/resnet50.pth"))
        else:
...
def get_torch_dataset(args, transform_type):
    ...
    if dataset == "cifar10":
        ...
        print(f"{data_path}")
        full_data = EuroSAT(root = data_path, download = False)
        print(f"full data:{full_data}")
        train_indices, val_indices = get_indices(full_data)
        train_set = Subset(EuroSAT(data_path, transform=train_transform, download=False), train_indices)
        test_set = EuroSAT(data_path, transform=test_transform, download=False)
        class_cnt = 10
        ...
        # 128
        train_set = StanfordCars(root=data_path, split='train', transform=train_transform, download=False)
        test_set = StanfordCars(root=data_path, split='test', transform=train_transform, download=False)
        class_cnt = 196
    
    elif dataset == 'flowers102':
        train_set = Flowers102(root=data_path, split='train', transform=train_transform, download=True)
        test_set = Flowers102(root=data_path, split='test', transform=train_transform, download=True)
        class_cnt = 102
    
    elif dataset == 'dtd':
        full_dataset = DTD(root=data_path, transform=train_transform, download=True)
