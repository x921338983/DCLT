...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, vis=False, head_init='uniform'):
    ...
    def load_from(self, weights):
        with torch.no_grad():
            if self.head_init == 'uniform':
                nn.init.uniform_(self.head.weight, -0.1, 0.1)
                nn.init.uniform_(self.head.bias, -0.1, 0.1)
            elif self.head_init == 'normal':
                nn.init.normal_(self.head.weight, mean=0.0, std=0.01)
                nn.init.normal_(self.head.bias, mean=0.0, std=0.01)
            elif self.head_init == 'xavier_uniform':
                nn.init.xavier_uniform_(self.head.weight)
                nn.init.xavier_uniform_(self.head.bias)
            elif self.head_init == 'kaiming_normal':
                nn.init.kaiming_normal_(self.head.weight, mode='fan_out', nonlinearity='relu')
                nn.init.kaiming_normal_(self.head.bias, mode='fan_out', nonlinearity='relu')
            elif self.head_init == 'zero':
                nn.init.zeros_(self.head.weight)
                nn.init.zeros_(self.head.bias)
            elif self.head_init == 'pretrain':
...
CONFIGS = {
    ...
    'vit_b_32': configs.get_b32_config(),
    'vit_l_16': configs.get_l16_config(),
    'vit_l_32': configs.get_l32_config(),
    'vit_h_14': configs.get_h14_config(),
    'r50_vit_b_16': configs.get_r50_b16_config(),
    'test': configs.get_test(),
}
